...
//			System.out.println("获得Excel表格的内容总共："+map.size()+"条记录：");
			for (int i = 1; i <= map.size(); i++) {
				try {
					...
					String key =  cellValue.get(0).toString().trim();
					String params =  cellValue.get(1).toString().trim();
					
					String realyParams = params.replaceFirst(params.split("&")[0]+"&", "");
					String realyURL = url+realyParams;
