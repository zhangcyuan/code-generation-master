...
*/
public class AISSTest {
	
	public static void main(String[] args) {
		//分类
		String url = "http://www.aiss8.net/?index-%d.htm"; 
		String baseUrl = "http://www.aiss8.net";
		//页码总数
		int pagenum = 14;
		//图片总数
		int counts = 0;
...
//				System.out.println(document.body());
				Elements els =	document.select("div.col-md-4").select("div.product-item").select("a[href*=thread]:lt(1)");
				for (Element element : els) {
					String modelUrl = baseUrl+element.attr("href");
					Document document2 = Jsoup.parse(new URL(modelUrl), 5000);
					Elements els2 =document2.select("pre:contains(down.aiss8.net)");
					for (Element element2 : els2) {
						counts++;
						System.out.println(counts+"="+element2.text());
						Thread.sleep(200);
					}
